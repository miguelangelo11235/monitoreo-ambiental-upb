...
class CLIMenu:
    ...
    async def test_connectivity(self) -> None:
        ...
        if current_uri:
            print(f"  Probando MongoDB en URI: {current_uri[:35]}...")
            ok = await asyncio.to_thread(MongoDBStorage.validate_connection, current_uri)
            if ok:
                print("  ✓ Conexión exitosa a MongoDB.")
            else:
                print("  ❌ Falló la conexión a MongoDB. Verifique credenciales, IP de la Raspberry o si la base local está activa.")
                change = await self._async_input("  ¿Desea configurar Credenciales de MongoDB (Usuario/Contraseña) en .env? [s/N]: ")
    ...
    async def configure_mongodb_wizard(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("         🗄️ CONFIGURACIÓN DE BASE DE DATOS MONGODB")
        print("=" * 70)
        ...
        print(f" URI Actual    : {current_uri[:45]}...")
        ...
        print(" 1. Ingresar Usuario y Contraseña (MongoDB Atlas - Recomendado)")
        print(" 2. Ingresar URI completa de MongoDB (Local o Atlas)")
        print(" 3. Cambiar intervalo de guardado en MongoDB (actual: %dm)" % settings.mongo_save_interval_min)
        print(" 4. Probar conexión a MongoDB ahora")
        ...
        if sub_choice == "1":
            ...
            target_uri = template.replace("<USER>", user).replace("<PASSWORD>", pwd)

            print("🔍 Validando conexión a MongoDB...")
            if await asyncio.to_thread(MongoDBStorage.validate_connection, target_uri):
                ...
                print("✓ ¡Conexión exitosa a MongoDB! URI guardada en .env\n")
            else:
                print("❌ No se pudo conectar a MongoDB. Verifique usuario, contraseña o lista blanca de IPs en Atlas.\n")

        elif sub_choice == "2":
            target_uri = await self._async_input("Ingrese URI completa de MongoDB: ")
            target_uri = target_uri.strip()
            if target_uri:
                print("🔍 Validando nueva URI...")
                if await asyncio.to_thread(MongoDBStorage.validate_connection, target_uri):
                    save_env_variable("MONGO_URI", target_uri)
                    settings.mongo_uri = target_uri
                    if self.mongo_storage:
                    ...
                    print("❌ No se pudo conectar con la URI ingresada.\n")
            ...
            if current_uri and await asyncio.to_thread(MongoDBStorage.validate_connection, current_uri):
                print(f"✓ ¡Conexión EXITOSA a MongoDB! (DB: {settings.mongo_db}, Colección: {settings.mongo_collection})\n")
            else:
                print("❌ FALLÓ la conexión a MongoDB. Verifique las credenciales e IP registrada en Atlas.\n")
